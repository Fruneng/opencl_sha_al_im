...
void crypt_all()
{
	...
	ret = clEnqueueWriteBuffer(command_queue, buffer_keys, CL_TRUE, 0, SHA1_PLAINTEXT_LENGTH * kpc, saved_plain, 0, NULL, NULL);
	printf("%s\n",buffer_keys);
	ret = clEnqueueNDRangeKernel(command_queue, kernel, 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);

	ret = clFinish(command_queue);
	// read back partial hashes
	ret = clEnqueueReadBuffer(command_queue, buffer_out, CL_TRUE, 0, sizeof(cl_uint) * SHA1_RESULT_SIZE, partial_hashes, 0, NULL, NULL);
	have_full_hashes = 0;
}
...
void create_clobj(){
	pinned_saved_keys = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, (SHA1_PLAINTEXT_LENGTH)*kpc, NULL, &ret);
	saved_plain = (char*)clEnqueueMapBuffer(command_queue, pinned_saved_keys, CL_TRUE, CL_MAP_WRITE | CL_MAP_READ, 0, (SHA1_PLAINTEXT_LENGTH)*kpc, 0, NULL, NULL, &ret);
	...
	pinned_partial_hashes = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeof(cl_uint) * SHA1_RESULT_SIZE, NULL, &ret);
	partial_hashes = (cl_uint *) clEnqueueMapBuffer(command_queue, pinned_partial_hashes, CL_TRUE, CL_MAP_READ, 0, sizeof(cl_uint) * SHA1_RESULT_SIZE, 0, NULL, NULL, &ret);
	memset(partial_hashes, 0, sizeof(cl_uint) * SHA1_RESULT_SIZE);

	buffer_keys = clCreateBuffer(context, CL_MEM_READ_ONLY, (SHA1_PLAINTEXT_LENGTH) * kpc, NULL, &ret);
	buffer_out = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(cl_uint) * SHA1_RESULT_SIZE, NULL, &ret);
